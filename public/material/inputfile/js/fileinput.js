...
(function (factory) {
    ...
    $h = {
        ...
        toggleFullScreen: function (maximize) {
            ...
            if (de && maximize && !$h.checkFullScreen()) {
                ...
                if (doc.exitFullscreenElement) {
                    doc.exitFullscreenElement();
    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            tActions = '<div class="file-upload-indicator" title="{indicatorTitle}">{indicator}</div>\n' +
                ...
                '</div>';
            //noinspection HtmlUnknownAttribute
            tActionDelete = '<button type="button" style="background-color:#920000;padding:6px;border-radius:15px;" class="kv-file-remove {removeClass}"' +
                'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';
            tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
                '{uploadIcon}</button>';
            tActionZoom = '<button type="button" style="background-color:#0d7188;padding:7px;border-radius:15px;" class="kv-file-zoom {zoomClass}" ' +
                'title="{zoomTitle}">{zoomIcon}</button>';
            tActionDrag = '<span class="file-drag-handle {dragClass}" title="{dragTitle}">{dragIcon}</span>';
            tTagBef = '<div class="file-preview-frame {frameClass}" id="{previewId}" style="border: 1px solid black !important;box-shadow: 5px 2px 5px #5a5a5a;border-radius: 11px;" data-fileindex="{fileindex}"' +
            ...
            tHtml = '<div class="kv-preview-data file-preview-html" title="{caption}" ' + $h.STYLE_SETTING +
                '>{data}</div>\n';
            tImage = '<img src="{data}" class="file-preview-image kv-preview-data" style="width:195px;height:160px;" title="{caption}" alt="{caption}" ' +
                $h.STYLE_SETTING + '>\n';
            ...
            self.defaults = {
                ...
                fileActionSettings: {
                    ...
                    showZoom: true,
                    showDrag: false,
                    removeIcon: '<i class="material-icons">delete_forever</i>',
                    removeClass: 'btn btn-xs btn-default',
                    removeTitle: 'Eliminar archivo',
                    uploadIcon: '<i class="material-icons">delete_forever</i>',
                    uploadClass: 'btn btn-xs btn-default',
                    uploadTitle: 'Subir Archivo',
                    zoomIcon: '<i class="material-icons">search</i>',
                    zoomClass: 'btn btn-xs btn-default',
                    zoomTitle: 'Ver Detalles',
                    dragIcon: '<i class="material-icons">search</i>',
                    dragClass: 'text-info',
                    dragTitle: ' ',
                    dragSettings: {},
            ...
            readFile = function (i) {
                ...
                if (fileCount === 0 && !$h.isEmpty(fileExt) && $h.isArray(fileExt) && !$h.isEmpty(fileExtExpr)) {
                    ...
                    if (fileCount === 0) {
                        ...
                        const box = document.getElementById('content_data');
                        const caja = document.getElementById('render_click');
                        var rnder="<a onclick='cancelbtn()' class='btn btn-red' style='background-color: brown; color:aliceblue'>Anular</a>";
                        box.remove();
                        caja.innerHTML=rnder;
                        self.isError = throwError(msg, file, previewId, i);
                        return;
                    }
                }
    ...
    $.fn.fileinput.defaults = {
        ...
        previewZoomButtonIcons: {
            toggleheader: '<i class="material-icons">format_strikethrough</i>',
            fullscreen: '<i class="material-icons">fullscreen</i>',
            borderless: '<i class="material-icons">fullscreen_exit</i>',
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgFilesTooLess: 'Tipo de archivo no válido. Sólo se permiten archivos de tipo JPG, JPEG, PNG.',
        ...
        dropZoneTitle: 'Arrastre y suelte aquí los archivos <br> <i style="font-size:30px;" class="fa fa-picture-o"></i> ',
